...
#region Public Functions
            public void PlayAudio(AudioType _type, bool _fade=false, float _fadeDuration=1f, float _delay=0.0F) {
                AddJob(new AudioJob(AudioAction.START, _type, _fade,_fadeDuration, _delay));
            }

            public void StopAudio(AudioType _type, bool _fade=false, float _fadeDuration=1f, float _delay=0.0F) {
                AddJob(new AudioJob(AudioAction.STOP, _type, _fade,_fadeDuration, _delay));
            }

            public void RestartAudio(AudioType _type, bool _fade=false, float _fadeDuration=1f, float _delay=0.0F) {
...
#region Private Functions
            ...
            private IEnumerator RunAudioJob(AudioJob _job) {
                ...
                ConfigureAudioSourceFromAudioTrack(_job.type, _track);
                switch (_job.action) {
                ...
                if (_job.fade) {
                    ...
                    float _target = _initial == 0 ? _track.source.volume : 0;
            ...
            private void ConfigureAudioSourceFromAudioTrack(AudioType _type, AudioTrack _track)
            {
                foreach (AudioObject _obj in _track.audio) {
                    if (_obj.type == _type)
                    {
                        _track.source.clip = _obj.clip;
                        _track.source.volume = _obj.volume;
                        _track.source.pitch = _obj.pitch;
                        _track.source.loop = _obj.loop;
                    }
                }
            }
